...
def download_exercises(json_dir, exercism_dir, track=None, force=False, delay=2):
        ...
        if filename.endswith(".json"):
            ...
            with open(filepath, "r", encoding="utf-8") as file:
                ...
                for exercise, status in exercises.items():
                    ...
                    if status[0] != "locked" and (
                        force or not os.path.exists(exercise_dir)
                    ):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="Force download even if exercise already exists",
    )
    parser.add_argument(
        "--delay",
        type=float,
    ...
    download_exercises(
        JSON_DIR, EXERCISM_DIR, track=args.track, force=args.force, delay=args.delay
    )
